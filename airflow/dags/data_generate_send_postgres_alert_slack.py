...
default_args = {
    ...
    'retries': 1,
    'on_failure_callback': task_failure_alert  # 작업 실패 시 콜백
}
...
with DAG(
    ...
    generate_fake_data_task = PythonOperator(
        ...
        op_args=[30],  # 30개의 데이터를 생성
        provide_context=True
    )

    # Task 2: PostgreSQL에 데이터 삽입
    ...
    # Task 3: Slack 알림 전송 (성공 시)
    slack_notification_task = PythonOperator(
        task_id='send_slack_notification',
        python_callable=lambda: send_slack_notification(),
        provide_context=True
    )
